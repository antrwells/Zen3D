...
void PPRenderer::CreateOutlineGP() {
    ...
    PSOCreateInfo.PSODesc.Name = "PPRenderer Outline PSO";
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        ShaderCI.Desc.Name = "PPRenderer - outline VS";
        ShaderCI.FilePath = "data/pp_outline.vsh";
        app->GetDevice()->CreateShader(ShaderCI, &pVS);
        // Create dynamic uniform buffer that will store our transformation matrix
        // Dynamic buffers can be frequently updated by the CPU
        BufferDesc CBDesc;
        CBDesc.Name = "VS constants CB";
        CBDesc.Size = sizeof(OutlineConstants);
        CBDesc.Usage = USAGE_DYNAMIC;
        CBDesc.BindFlags = BIND_UNIFORM_BUFFER;
        CBDesc.CPUAccessFlags = CPU_ACCESS_WRITE;
        app->GetDevice()->CreateBuffer(CBDesc, nullptr, &m_VSConstants_Outline);
    }
    ...
    RefCntAutoPtr<IShader> pPS;
    {
        ...
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "PPRenderer - Outline PS";
        ShaderCI.FilePath = "data/pp_outline.psh";
    ...
    app->GetDevice()->CreateGraphicsPipelineState(PSOCreateInfo, &m_pPSO_Outline);

    // Since we did not explcitly specify the type for 'Constants' variable, default
    // type (SHADER_RESOURCE_VARIABLE_TYPE_STATIC) will be used. Static variables never
    // change and are bound directly through the pipeline state object.
    m_pPSO_Outline->GetStaticVariableByName(SHADER_TYPE_VERTEX, "Constants")->Set(m_VSConstants_Outline);

    // Create a shader resource binding object and bind all static resources in it
    m_pPSO_Outline->CreateShaderResourceBinding(&m_pSRB_Outline, true);
...
void PPRenderer::RenderOutline(Texture2D* texture, int w, int h,float diff,float inc,float4 color) {

    OutlineConstants consts;
   ...
   // co_const.lightProps = float4(light->GetRange(), 0, 0, 0);
    ...
    consts.outlineCol = color;
  

    MapHelper<OutlineConstants> CBConstants(Application::GetApp()->GetContext(), m_VSConstants_Outline, MAP_WRITE, MAP_FLAG_DISCARD);
    *CBConstants = consts;

    m_pSRB_Outline->GetVariableByName(SHADER_TYPE_PIXEL, "g_Texture")->Set(texture->GetView(), SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
    ...
    mDraw->End(m_pPSO_Outline, m_VSConstants_Outline, m_pSRB_Outline);
