...
uniform sampler2D tNorm1;
uniform sampler2D tTex2;
uniform sampler2D tNorm2;
...
void main(){
    ...
    vec3 normal = vec3(0.5,0.5,1);
      ...
      normal = texture2D(tNorm1,uv).rgb;
    ...
    vec3 cTex = texture(tTex1,uv).rgb;
    ...
    mt1 = texture(tTex1,uv).rgb * tt;
    mt2 = texture(tTex2,uv).rgb * (1.0-tt);

    fc.rgb *= mt1+mt2;
    

//    fc.rgb = vec3(tt,tt,tt);
